...
SRC_URI="https://launchpad.net/${PN}/${SERIES}/${PV}/+download/${P}.tar.xz
	!minimal? (
		https://github.com/KiCad/kicad-symbols/archive/${PV}.tar.gz -> symbols.tar.gz
		https://github.com/KiCad/kicad-footprints/archive/${PV}.tar.gz -> footprints.tar.gz
		https://github.com/KiCad/kicad-packages3D/archive/${PV}.tar.gz -> packages3D.tar.gz
	)
...
COMMON_DEPEND="
	>=x11-libs/wxGTK-3.0.2:${WX_GTK_VER}[X,opengl]
	python? (
...
src_install() {
	...
	#Install symbols, footprints und 3dpackages when not choosen miniaml install.
	if ! use minimal; then
		dodir /usr/share/kicad/template
		dodir /usr/share/kicad/library
		dodir /usr/share/kicad/modules
		dodir /usr/share/kicad/modules/packages3d
		
		insinto /usr/share/kicad/library/
		doins "${WORKDIR}"/symbols/*.lib
		doins "${WORKDIR}"/symbols/*.dcm 
		
		insinto /usr/share/kicad/modules/
		doins -r "${WORKDIR}"/footprints/*.pretty
		
		insinto /usr/share/kicad/modules/packages3d/
		doins -r "${WORKDIR}"/packages3D/*.3dshapes
		
		insinto /usr/share/kicad/template
		doins "${WORKDIR}"/symbols/sym-lib-table
		doins "${WORKDIR}"/footprints/fp-lib-table
	fi
}		
